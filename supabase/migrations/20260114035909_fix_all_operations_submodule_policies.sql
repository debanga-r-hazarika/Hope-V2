/*
  # Fix All Operations Sub-Module RLS Policies
  
  Updates all RLS policies for Operations sub-modules to check for the correct
  sub-module names instead of just 'operations'. This ensures:
  - Read-Write users can Edit/Add/Remove
  - Read-Only users can only view (no changes)
  - No-Access users can't see or access
...
CREATE POLICY "Users with read-write access can insert batch outputs"
  ...
  WITH CHECK (
    ...
    OR (
      ...
      -- Note: batch_id in WITH CHECK refers to the new row's column value
      -- Use explicit UUID casting to avoid type mismatch
      AND COALESCE((
        SELECT pb.is_locked FROM production_batches pb WHERE pb.id = batch_id::uuid
      ), false) = false
    )
  );
