/*
  # Update Delivery Trigger to Match Display Calculation
  
  This migration updates the delivery trigger to use the same calculation as the UI:
  actual_available = quantity_created - delivered (without subtracting reservations)
  
  This ensures consistency between the UI display and database validation.
*/
...
BEGIN
  ...
  IF delivery_difference != 0 THEN
    ...
    IF delivery_difference > 0 THEN
      ...
      -- Update quantity_available to match the calculation: quantity_created - total_delivered
      -- This ensures it always matches our display calculation and won't go negative
      -- Use GREATEST to ensure it never goes below 0 (satisfies check constraint)
      UPDATE processed_goods
      SET quantity_available = GREATEST(0, quantity_created_val - total_delivered_val)
      WHERE id = NEW.processed_good_id;
    ...
    IF delivery_difference < 0 THEN
      -- Update quantity_available to match the calculation: quantity_created - total_delivered
      -- This ensures it always matches our display calculation
      -- Use GREATEST to ensure it never goes below 0 (satisfies check constraint)
      UPDATE processed_goods
      SET quantity_available = GREATEST(0, quantity_created_val - total_delivered_val)
      WHERE id = NEW.processed_good_id;
