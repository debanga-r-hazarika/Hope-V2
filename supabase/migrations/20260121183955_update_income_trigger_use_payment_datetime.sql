/*
  # Update income trigger to use payment_datetime

  This migration updates the create_income_from_payment trigger to use the
  payment_datetime column (which contains the exact date/time selected by the user)
  instead of created_at (when the payment was saved to database).

  This ensures income entries show the correct payment date/time as selected
  by the user in the payment form.
*/

-- Update the trigger function to use payment_datetime for payment_at
...
BEGIN
  ...
  VALUES (
    ...
    paid_to_user_value,
    NEW.payment_date,
    COALESCE(NEW.payment_datetime, NEW.created_at), -- Use payment_datetime if available, fallback to created_at
    payment_method_map,
